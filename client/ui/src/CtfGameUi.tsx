...
export default function CtfGameUi({ engine }: { engine: Engine }) {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      const freshWorldState = engine.ctx_get_world_state();
      ...
      const freshPlayerInfo = engine.ctx_get_players().get(playerId);
  ...
  const ammoLowClass = playerAmmo <= 3 ? "low" : "";
  const healthLowClass = playerHealth <= 1 ? "low" : "";
  return (
    <div className={"ctf team-" + playerTeam}>
      <div className="status-ctr">
        <div className={"status status-health " + healthLowClass}>
          <span>{playerHealth}</span>
        </div>
        <div className={"status status-ammo " + ammoLowClass}>
          <span>{playerAmmo}</span>
        </div>
      </div>
      ...
      <div className="instructions-ctr">
        <p className="instruction-whichteam">
          You are on <span className={"team-" + playerTeam}>{playerTeam}</span> team.
        </p>
